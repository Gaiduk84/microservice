        ...
        try {
            ...
            if (userRepresentation != null) {
                ...
                if (userResource.roles() != null) {
                    userRoles = userResource.roles().getAll().getRealmMappings();
                } else {
                    userRoles = Collections.emptyList();
                }
                userGroups = userResource.groups();
            } else {
                throw new BackendResourcesException("User not found", HttpStatus.NOT_FOUND); // Генерируем исключение, если пользователь не найден
            }
